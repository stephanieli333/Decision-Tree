...
public class DecisionTree implements Serializable {
	...
	DTNode rootDTNode;
	int minSizeDatalist; //minimum number of datapoints that should be present in the dataset to initiate a split
	...
	class DTNode implements Serializable{
		...
		// Given a datalist, this method returns the label that has the most occurences
		// In case of a tie it returns the label with the smallest value in the tie
		int findMajority(ArrayList<Datum> datalist)
		{
			int l = datalist.get(0).x.length;
			int [] votes = new int[l];

			//loops through the data and count the occurrences of datapoints of each label
			...
			int max = -1;
			int max_index = -1;
			// finds the label with the max occurrences
			for (int i = 0 ; i < l ;i++)
			{
				if (max<votes[i])
				{
		...
		// takes in a datapoint (excluding the label) in the form of an array of type double (Datum.x) and
